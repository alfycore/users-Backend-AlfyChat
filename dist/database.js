...
async function runMigrations(db) {
    ...
    await db.execute(`CREATE TABLE IF NOT EXISTS custom_badges (
      id VARCHAR(36) PRIMARY KEY,
    ...
    await db.execute(`INSERT IGNORE INTO site_settings (setting_key, setting_value) VALUES ('registration_enabled', 'true')`);
    await db.execute(`INSERT IGNORE INTO site_settings (setting_key, setting_value) VALUES ('turnstile_enabled', 'false')`);
    ...
    await db.execute(`CREATE TABLE IF NOT EXISTS signal_key_bundles (
      user_id VARCHAR(36) PRIMARY KEY,
    ...
    try {
        await db.execute(`ALTER TABLE signal_key_bundles
    ...
    await db.execute(`CREATE TABLE IF NOT EXISTS signal_one_time_prekeys (
    ...
    for (const sql of twoFaAlterations) {
        try {
            ...
            if (e?.errno !== 1060)
                console.log('2FA migration warning:', e?.message);
        }
    }
    // ==========================================
    ...
    for (const sql of e2eeAlterations) {
        try {
            ...
            if (e?.errno !== 1060)
                console.log('E2EE migration warning:', e?.message);
        }
    }
    // ==========================================
    ...
    // ==========================================
    {
        ...
        for (const [colName, sql] of alterCols) {
            const [existing] = await db.query('SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=? AND COLUMN_NAME=?', ['users', colName]);
            if (!existing.length) {
                try {
                    await db.execute(sql);
                }
    ...
    // CHANGELOGS
    // ==========================================
    await db.execute(`CREATE TABLE IF NOT EXISTS changelogs (
      id VARCHAR(36) PRIMARY KEY,
    ...
    try {
        ...
        if (e?.errno !== 1060)
            console.log('[DB] changelogs.banner_url already exists');
    }
    // ==========================================
    ...
    for (const sql of newUserCols) {
        try {
            ...
            if (e?.errno !== 1060)
                console.log('User new cols migration warning:', e?.message);
        }
    }
    // activity_visibility_exceptions: masquer le statut en ligne à certains users
    await db.execute(`CREATE TABLE IF NOT EXISTS activity_visibility_exceptions (
      user_id VARCHAR(36) NOT NULL,
    ...
    // ==========================================
    await db.execute(`CREATE TABLE IF NOT EXISTS helpdesk_tickets (
      id VARCHAR(36) PRIMARY KEY,
    ...
    await db.execute(`CREATE TABLE IF NOT EXISTS helpdesk_messages (
      id VARCHAR(36) PRIMARY KEY,
    ...
    try {
        ...
        if (e?.errno !== 1060)
            console.log('[DB] role enum migration:', e?.message);
    }
    // ==========================================
  ...
  `);
    ...
    try {
        ...
        if (e?.errno !== 1265 && e?.errno !== 1292)
            console.log('[DB] helpdesk requester_id:', e?.message);
    }
    try {
        ...
        if (e?.errno !== 1060)
            console.log('[DB] helpdesk requester_email already exists');
    }
    ...
    for (const [id, slug, title, desc, icon, color, order] of cats) {
        await db.execute(`INSERT IGNORE INTO support_categories (id, slug, title, description, icon_name, color, sort_order) VALUES (?,?,?,?,?,?,?)`, [id, slug, title, desc, icon, color, order]);
    }
    // Articles par défaut — Sécurité
    const articlesSecurite = [
    ...
    for (const [id, slug, title, summary, content, tags, order] of articlesSecurite) {
        await db.execute(`INSERT IGNORE INTO support_articles (id, category_id, slug, title, summary, content, tags, sort_order) VALUES (?,?,?,?,?,?,?,?)`, [id, 'cat-securite', slug, title, summary, content, tags, order]);
    }
    // Articles par défaut — Compte
    const articlesCompte = [
    ...
    for (const [id, slug, title, summary, content, tags, order] of articlesCompte) {
        await db.execute(`INSERT IGNORE INTO support_articles (id, category_id, slug, title, summary, content, tags, sort_order) VALUES (?,?,?,?,?,?,?,?)`, [id, 'cat-compte', slug, title, summary, content, tags, order]);
    }
    // Articles par défaut — Serveurs
    const articlesServeurs = [
    ...
    for (const [id, slug, title, summary, content, tags, order] of articlesServeurs) {
        await db.execute(`INSERT IGNORE INTO support_articles (id, category_id, slug, title, summary, content, tags, sort_order) VALUES (?,?,?,?,?,?,?,?)`, [id, 'cat-serveurs', slug, title, summary, content, tags, order]);
    }
    // Annonces initiales
    const announcements = [
    ...
    for (const [id, type, title, summary, pubDate, resolved] of announcements) {
        await db.execute(`INSERT IGNORE INTO support_announcements (id, type, title, summary, is_resolved, published_at) VALUES (?,?,?,?,?,?)`, [id, type, title, summary, resolved, pubDate]);
    }
    ...
    for (const [id, title, status, catLabel] of knownIssues) {
        await db.execute(`INSERT IGNORE INTO support_known_issues (id, title, status, category_label) VALUES (?,?,?,?)`, [id, title, status, catLabel]);
    }
}
