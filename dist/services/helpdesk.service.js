...
class HelpdeskService {
    ...
    async getTickets(filters = {}) {
       ...
       ORDER BY
         ...
         t.created_at DESC
       LIMIT ? OFFSET ?`, params);
        return rows.map(r => this.formatTicket(r));
    }
    ...
    async createTicket(data) {
        ...
        await this.db.execute(`INSERT INTO helpdesk_tickets (id, subject, description, priority, category, requester_id, requester_email)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, [id, data.subject, data.description, data.priority ?? 'medium', data.category ?? 'general', data.requesterId ?? null, data.requesterEmail ?? null]);
    ...
    async updateTicket(id, data) {
        ...
        if (updates.length === 0)
            return;
        params.push(id);
        await this.db.execute(`UPDATE helpdesk_tickets SET ${updates.join(', ')} WHERE id = ?`, params);
    }
    async deleteTicket(id) {
        await this.db.execute('DELETE FROM helpdesk_tickets WHERE id = ?', [id]);
    }
    // ── Messages ───────────────────────────────────────────────────────────────
    async getMessages(ticketId, opts = {}) {
        const whereExtra = opts.excludeInternal ? ' AND m.is_internal = FALSE' : '';
        const [rows] = await this.db.query(`SELECT m.*,
    ...
    async addMessage(ticketId, authorId, content, isInternal = false) {
        const id = (0, uuid_1.v4)();
        await this.db.execute(`INSERT INTO helpdesk_messages (id, ticket_id, author_id, content, is_internal) VALUES (?, ?, ?, ?, ?)`, [id, ticketId, authorId, content, isInternal]);
        // Auto-transition: si le ticket est 'open', passer en 'in_progress' quand le staff répond
        await this.db.execute(`UPDATE helpdesk_tickets SET status = 'in_progress', updated_at = NOW()
       WHERE id = ? AND status = 'open'`, [ticketId]);
        const [rows] = await this.db.query(`SELECT m.*,
        COALESCE(u.display_name, u.username) AS author_name,
        u.username AS author_username,
        u.role AS author_role
       FROM helpdesk_messages m LEFT JOIN users u ON u.id = m.author_id WHERE m.id = ?`, [id]);
        return this.formatMessage(rows[0]);
    }
    // ── Staff agents ───────────────────────────────────────────────────────────
    async getStaffAgents() {
        const [rows] = await this.db.query(`SELECT id, username, COALESCE(display_name, username) AS display_name, role, avatar_url, is_online
       FROM users
...
exports.helpdeskService = new HelpdeskService();
