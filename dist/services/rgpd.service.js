...
exports.rgpdService = exports.RgpdService = void 0;
...
class RgpdService {
    ...
    async exportUserData(userId) {
        ...
        const [userRows] = await this.db.query(`SELECT id, email, username, display_name, avatar_url, bio, status, 
              created_at, last_seen_at
       FROM users WHERE id = ?`, [userId]);
        const [prefRows] = await this.db.query('SELECT * FROM user_preferences WHERE user_id = ?', [userId]);
        const [messageRows] = await this.db.query(`SELECT id, content, channel_id, recipient_id, created_at
       FROM messages WHERE author_id = ? ORDER BY created_at DESC`, [userId]);
        const [friendRows] = await this.db.query(`SELECT f.id, f.status, f.created_at,
              u.username, u.display_name
       FROM friends f
       JOIN users u ON (
         CASE WHEN f.user_id = ? THEN f.friend_id ELSE f.user_id END = u.id
       )
       WHERE f.user_id = ? OR f.friend_id = ?`, [userId, userId, userId]);
        const [serverRows] = await this.db.query(`SELECT s.id, s.name, sm.joined_at, sm.nickname
    ...
    async requestDeletion(userId, deleteMessages = false) {
        const scheduledDeletionAt = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000); // 30 jours
        // Vérifier si une demande existe déjà
        const [existing] = await this.db.query('SELECT id FROM rgpd_deletion_requests WHERE user_id = ? AND completed_at IS NULL', [userId]);
        if (existing.length > 0) {
            // Mettre à jour la demande existante
            await this.db.execute('UPDATE rgpd_deletion_requests SET scheduled_deletion_at = ?, requested_at = NOW() WHERE user_id = ? AND completed_at IS NULL', [scheduledDeletionAt, userId]);
        }
        else {
            // Créer une nouvelle demande
            await this.db.execute(`INSERT INTO rgpd_deletion_requests (id, user_id, scheduled_deletion_at)
         VALUES (?, ?, ?)`, [(0, uuid_1.v4)(), userId, scheduledDeletionAt]);
        }
        ...
        if (deleteMessages) {
            ...
            try {
                await this.db.execute(`UPDATE messages SET content = '[Message supprimé par l\'utilisateur]', sender_content = NULL, is_deleted = TRUE WHERE sender_id = ?`, [userId]);
            }
            catch {
    ...
    async cancelDeletion(userId) {
        await this.db.execute('DELETE FROM rgpd_deletion_requests WHERE user_id = ? AND completed_at IS NULL', [userId]);
    }
    // Récupérer les consentements
    async getConsents(userId) {
        const [rows] = await this.db.query('SELECT id, consent_type, granted, granted_at, revoked_at FROM rgpd_consents WHERE user_id = ?', [userId]);
        return rows.map(row => ({
            id: row.id,
    ...
    async updateConsent(userId, consentType, granted) {
        const [existing] = await this.db.query('SELECT id FROM rgpd_consents WHERE user_id = ? AND consent_type = ?', [userId, consentType]);
        if (existing.length > 0) {
            if (granted) {
                await this.db.execute(`UPDATE rgpd_consents 
         ...
         VALUES (?, ?, ?, ?, NOW())`, [(0, uuid_1.v4)(), userId, consentType, granted]);
        }
    }
    // Anonymiser les données utilisateur
    async anonymizeUser(userId) {
        const anonymousUsername = `deleted_${(0, uuid_1.v4)().slice(0, 8)}`;
       ...
       WHERE id = ?`, [anonymousUsername, userId]);
        // Anonymiser les messages (garder pour le contexte mais supprimer le contenu)
        await this.db.execute(`UPDATE messages SET content = '[Message supprimé]' WHERE author_id = ?`, [userId]);
...
exports.rgpdService = new RgpdService();
//# sourceMappingURL=rgpd.service.js.map